...
object BuiltinPaletteActions {
    ...
    fun register(registry: PaletteActionRegistry) {
        ...
        registry.register(screenAction("options", "Open Options", "Open Minecraft options") {
            it.minecraft.setScreen(OptionsScreen(parentScreen(it), it.minecraft.options))
        })
        registry.register(screenAction("controls", "Open Controls", "Edit Minecraft and mod key mappings") {
            it.minecraft.setScreen(ControlsScreen(parentScreen(it), it.minecraft.options))
        })
        registry.register(screenAction("mods", "Open Mods", "Open the NeoForge mod list") {
            it.minecraft.setScreen(ModListScreen(parentScreen(it)))
        })
        registry.register(screenAction("close_screen", "Close Current Screen", "Return to the game or previous context") {
